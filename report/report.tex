...
\subsection{C}
For a while, C was the \emph{lingua franca} of the programming world. C is a fairly old language, dating back to 1972 and was used to write many of the UNIX utilities. %TODO REFS!

C is technically considered a high-level programming language, in that it gives you automatic variable allocation and permits arbitrarily complex expressions without explicitly storing intermediary results - the detail of a register is seldom expressed in C code; variables always have an address in memory (unless the register keyword is used in variable declaration).
